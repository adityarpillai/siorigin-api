...
Parses FCE data for MSXML files downloaded from http;//cmu.smartevals.com

MSXML is used to compensate for a bug in the FCE export feature: The first few
lines of any output are cut off. MSXML is the only format to have enough useless
metadata at the beginning of the file to prevent important data from being
cutoff.
...
def parse_fces(path):
    ...
    Parses FCE data froma a CSV file to JSON.
    
    Args:
        path (string): File location of CSV to parse from
    
    Returns:
        JSON: FCE data as a JSON
    """
    results = []
    categories = []
